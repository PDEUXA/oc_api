...
class TestStudent(MockedDoc):
    ...
    async def test_create_student(self):
        ...
        assert await create_student(UserModel(**data)) == UserModel()
    ...
    async def test_find_student_with_id(self):
        ...
        assert UserOutModel(**data) == await find_student_with_id(data["id"], self.mongodb_test)
        assert await find_student_with_id(9999) == UserOutModel()
        await delete_student(data["id"], self.mongodb_test)

    @pytest.mark.asyncio
    async def test_fetch_all_students(self):
        response = [UserOutModel(**student.dict()) for student in self.multiple_student]
    ...
    async def test_get_student_all_sessions(self):
        ...
        all_7777 = [SessionOutModel(**session) for session in self.multiple_session
                    if session["recipient"] == 7777]

        assert all_7777 == await get_student_all_sessions(7777, mongo=self.mongodb_test)

        completed_7777 = [SessionOutModel(**session) for session in self.multiple_session
        ...
        all_7777_except_canceled = [SessionOutModel(**session) for session in self.multiple_session
        ...
        all_7777_late = [SessionOutModel(**session) for session in self.multiple_session
